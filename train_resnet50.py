...
def stratified_split_indices(
    ...
    for c in classes:
        ...
        # If a class is extremely small, ensure val/test aren't empty when possible
...
def get_data_dirs(data_root: str) -> Tuple[str, Optional[str]]:
    """
    Supports:
    1) data_root/train + data_root/test
    2) data_root is itself the class-folder root (no train/test) -> we'll split
    3) user passes /content/train directly -> we'll split
    """
    ...
    # otherwise treat data_root as class-folder root
...
def freeze_all_except(model: nn.Module, trainable_names: List[str]) -> None:
    for n, p in model.named_parameters():
        p.requires_grad = any(n.startswith(prefix) for prefix in trainable_names)
...
def main():
    ...
    # splits when no explicit test folder
    parser.add_argument("--split_train", type=float, default=0.64)  # 64% train
    parser.add_argument("--split_val", type=float, default=0.16)    # 16% val
    parser.add_argument("--split_test", type=float, default=0.20)   # 20% test
    ...
    # Transforms (moderate-strong augmentation)
    ...
    full_train_eval = datasets.ImageFolder(train_dir, transform=eval_tf)  # same indices, eval transform
    ...
    if test_dir is not None:
        ...
        # split within train_dir into train/val/test (64/16/20 default), stratified
        targets = full_train.targets
    ...
    # Build sampler (on train subset targets)
    train_subset_targets = [targets[i] for i in train_idx]
    sampler = None
    if args.use_weighted_sampler:
        sampler = build_weighted_sampler(train_subset_targets, num_classes)
    ...
    if best0.get("top1", 0.0) >= args.target_acc:
        print("Target reached in head stage.")

    # Stage 1: unfreeze layer4 + fc
    freeze_all_except(model, ["layer4", "fc"])
    best1 = run_stage(
        "finetune_last_block",
        model,
        train_loader, val_loader,
    ...
    freeze_all_except(model, ["layer3", "layer4", "fc"])
    best2 = run_stage(
    ...
    # Pick best val among stages (based on val_top1)

...
def build_weighted_sampler_sqrt(train_targets: List[int], num_classes: int) -> WeightedRandomSampler:
    ...
    class_w = 1.0 / np.sqrt(class_counts)
...
def freeze_all_except(model: nn.Module, trainable_prefixes: List[str]) -> None:
    for n, p in model.named_parameters():
        p.requires_grad = any(n.startswith(prefix) for prefix in trainable_prefixes)
...
def run_stage(
    ...
    scaler = AmpGradScaler() if (amp and device.type == "cuda") else None
...
def main():
    ...
    parser.add_argument("--epochs_last", type=int, default=25)
    parser.add_argument("--lr_last", type=float, default=1e-4)
    parser.add_argument("--epochs_last2", type=int, default=20)
    parser.add_argument("--lr_last2", type=float, default=1e-5)
    ...
    if test_dir is not None:
        ...
        train_idx, val_idx, test_idx = stratified_split_indices(
            targets, train_ratio=args.split_train, val_ratio=args.split_val, test_ratio=args.split_test, seed=args.seed
        )
    ...
    if args.use_weighted_sampler:
        sampler = build_weighted_sampler_sqrt(train_subset_targets, num_classes)
    ...
    best_path = os.path.join(os.getcwd(), "best_densenet121_v2.pt")
    ...
    # Stage 1: finetune last block（denseblock4 + norm5 + classifier）
    ...
    # Stage 2: finetune last2 blocks（denseblock3+4 + norm5 + classifier）
    freeze_all_except(model, ["features.denseblock3", "features.transition3", "features.denseblock4", "features.norm5", "classifier"])
    best2 = run_stage(
